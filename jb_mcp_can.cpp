/*
  ...
  Updated by James.... 19/01/2022
    -> Increased timeout from 50 to 250.
	-> 
...
{
    INT8U res, i, ctrlval;
    INT8U ctrlregs[MCP_N_TXBUFFERS] = { MCP_TXB0CTRL, MCP_TXB1CTRL, MCP_TXB2CTRL };  // 0x30, 0x40 and 0x40
    ...
    *txbuf_n = 0x00;
	...
	{
        ctrlval = mcp2515_readRegister( ctrlregs[i] );					// read the txb register
        if ( (ctrlval & MCP_TXB_TXREQ_M) == 0 ) 						// and with 0x08 / %1000 TXREQ bit - 0=not pending, 1=pending tx
		{		
            *txbuf_n = ctrlregs[i]+1;                                   // return SIDH-address of Buffer
			//*txbuf_n = ctrlregs[i];                                       // return SIDH-address of this buffer
            res = MCP2515_OK;
...
INT8U MCP_CAN::sendMsg()
{
	...
	const INT8U MSG_WAIT = 99;

	res1 = MSG_WAIT;	// unknown state
	
	// get next free buffer from mcp2515
	// 0x30, 0x40 or 0x50 indicate a free buffer.  2 is a fail / all busy
    do {
    ...
    {   
        return CAN_GETTXBFTIMEOUT;                                      // (6) get tx buff time out         
    }
    uiTimeOut = 0;
    mcp2515_write_canMsg(txbuf_n);
    mcp2515_modifyRegister(txbuf_n-1, MCP_TXB_TXREQ_M, MCP_TXB_TXREQ_M);
    
	// poll the buffer, until sent, error or timeout.
    do
    {
        uiTimeOut++;
        res = mcp2515_readRegister(txbuf_n-1);                         // read send buff ctrl reg
		
        if((res & 0x08) == 0x00)  { 								   // bit 3 and with 0x08 / %1000 - TXREQ bit. 0=free, 1=pending
			res1=CAN_OK;											   // return CAN_OK (0) if 0
		}
		
		if((res & 0x10) == 0x10) {									   // bit 4 error bit = 1 bus error, 0 = no error
			res1=CAN_FAILTX;									       // return CAN_FAILTX (6)
		}
		
		if((res & 0x20) == 0x20) {									   // bit 5 lost arbration.  0=ok, 1=lost (not acknowledged)
			res1=CAN_NOMSG;											   // return CAN_NOMSG (4)
		}
		
    } while (res1==MSG_WAIT && (uiTimeOut < TIMEOUTVALUE));   
    
    if(uiTimeOut == TIMEOUTVALUE)                                       // (7) send msg timeout             	
